...
impl Product {
    pub async fn find_all(pool: &PostgresPool) -> Result<Vec<Product>> {
        let product = sqlx::query_as!(
            Product,
            r#"
              SELECT id, name, price, origin, cultivar, updated_at, created_at
                  FROM products
              ORDER BY updated_at
            "#
        )
    ...
    pub async fn create(input: ProductInput, pool: &PostgresPool) -> Result<Product> {
        ...
        let product = sqlx::query_as!(
            Product,
            r#"
              INSERT INTO products (name, price, origin, cultivar) VALUES ($1, $2, $3, $4)
                RETURNING id, name, price, origin, cultivar, updated_at, created_at
            "#,
            input.name,
            input.price,
            input.origin,
    ...
    pub async fn update(id: i32, input: ProductInput, pool: &PostgresPool) -> Result<Product> {
        ...
        let product = sqlx::query_as!(
            Product,
            r#"
              UPDATE products SET name = $1, price = $2, origin = $3 , cultivar = $4 WHERE id = $5
                RETURNING id, name, price, origin, cultivar, updated_at, created_at
            "#,
            input.name,
            input.price,
